...
Open `public/index.html` in any modern browser to use Siphon locally.
...
2. Click **Sync** (or it syncs automatically on page load and each feed refresh)
...
Siphon stores everything in your browser's localStorage. No data is sent to any server other than the feed sources themselves (proxied through the Cloudflare Worker when deployed). If you enable sync, your feed list and read state are stored in Cloudflare Workers KV, keyed by passphrase hash.
